...
export const XWing = ({}: XWingProps) => {
  ...
  useFrame(({ clock, mouse }) => {
    ...
    ref.current.position.lerp(
      new Vector3(x, y * 3, ref.current.position.z),
      0.1
    );
    ref.current.rotation.x =
      -x * 0.05 + Math.sin(clock.getElapsedTime()) * 0.25;
    ref.current.position.y += Math.sin(clock.getElapsedTime()) * 0.025;
    ref.current.position.z += Math.sin(clock.getElapsedTime()) * 0.025;
